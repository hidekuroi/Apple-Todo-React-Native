...
import { IsAuthResponseType, authAPI } from './../../api/auth-api';
...
export const authCheck = () => {
    return async (dispatch: AppDispatch, getState: () => RootState) => {
        ...
        if(response.data.resultCode === 0) dispatch(authUserSetted({...response.data.data, isAuth: true}))
        else dispatch(authUserSetted({email: '', id: 0, isAuth: false, login: ''}))
